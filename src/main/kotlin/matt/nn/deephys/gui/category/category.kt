	...
	v {
	  ...
	  when (selection) {
		...
		is CategoryConfusion -> {
		  deephysLabel(
			"Accuracy of ${selection.first.label}: ${CategoryAccuracy(selection.first, testLoader).formatted()}"
		  )
		  deephysLabel(
			"Accuracy of ${selection.second.label}: ${CategoryAccuracy(selection.second, testLoader).formatted()}"
		  )
	  ...
	  h {
		...
		v {
		  ...
		  /*maxWidthProperty.bindWeakly(viewer.widthProperty*0.45)*/
		  +CategoryPie(
			"False Positives (${allFalsePositives.size})",
			cats,
		...
		v {
		  alignment = TOP_CENTER
		  +CategoryPie(
			"False Negatives (${allFalseNegatives.size})",
			cats,
			nums = cats.associateWith { cat ->
